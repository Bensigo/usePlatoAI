...
- `P0` and `P1` findings are eligible for unattended follow-up issues.
- `P2` and `P3` findings should stay in the review unless the maintainer explicitly wants a tracking issue.
...
If there are unattended follow-up findings, put one object per issue in `fix_issues`:
...
Do not create JSON issue entries for vague preferences or non-blocking comments.
